...
public class AttrServiceImpl extends ServiceImpl<AttrDao, AttrEntity> implements AttrService {
    ...
    @Override
    public PageVo queryPageCatelogBaseAttrs(QueryCondition queryCondition, Long catId, Integer attr_type) {
        ...
        //TODO
        wrapper.eq("catelog_id",catId).eq("attr_type",attr_type);
        IPage<AttrEntity> data = this.page(page, wrapper);
        return new PageVo(data);
    }
    @Transactional
    @Override
    public void saveAttrRelation(AttrSaveVo attr) {
        //1.先把属性保存在属性表中，
